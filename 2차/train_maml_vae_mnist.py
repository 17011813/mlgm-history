...
def main():
    metasampler = MnistMetaSampler(
        batch_size=5,   # 이걸 train 할때는 키우면 안되고, test 할때는 20에서 100정도로 키워도 되나? 그래야 의미있는 성능 평가 아닌가..?
        meta_batch_size=4,
    ...
    with tf.Session() as sess:
        ...
        maml = Maml(
            ...
            sess,
            logger,
            num_updates=9,   # 20말고 6정도로 줄여 -- 과적합땜에
            update_lr=0.0005,
            meta_lr=0.0001,
            outliers_fraction=0.07)   # threshold 설정 비율
        """
        maml.train( train_itr=2000, test_itr=1, test_interval=100)
        """              
        maml.test( test_itr=1, restore_model_path='./data/maml_vae/maml_vae_19_23_01_18_22/itr_1900' )               
