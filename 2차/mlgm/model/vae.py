...
class Vae(Model):
    ...
    def build_loss(self, labels, logits):      # 복원된 예측 logits 결과로 라벨과 비교해서 loss 구함    # KL은 (5,)
        ...
        all_loss = tf.reduce_sum(tf.square(labels-logits), 1)       # all_loss가 reconstruction error임 -- 얘까지는 (5, 1)인데
        loss = tf.reduce_mean(all_loss + KL_divergence)        # MLP_VAE 식으로 loss 구하는거 바꿈 (MSE + KLD)  -- 얘부터 () 꼴
        test_loss = tf.reshape(all_loss, shape = [5,]) + KL_divergence    # (5,)로 각각 loss 출력함
