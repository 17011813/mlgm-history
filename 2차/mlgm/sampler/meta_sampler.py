...
class MetaSampler:
    ...
    def _gen_metadata(self, handle):
        num_inputs_per_batch = (self._batch_size)      # 배치마다 들어가는 입력 수 = batch_size(5) * 2
        ...
        for i in range(self._meta_batch_size):
            batch_input_sym = meta_batch_sym[0][i * num_inputs_per_batch:(i + 1) * num_inputs_per_batch]   # (10, 10)
        ...
        return tf.stack(all_input_batches), tf.stack(all_label_batches)   # (7, 10, 10) (7, 10) 여기 라벨은 실제 0 또는 1의 라벨           
