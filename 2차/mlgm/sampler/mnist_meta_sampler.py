...
class MnistMetaSampler(MetaSampler):
    def __init__(
        ...
        self.train_inputs, train_labels = scaler.fit_transform(np.array(pd.read_csv('./cover_train.csv'))), np.array(pd.read_csv('./cover_train_label.csv')).reshape(-1,)
        self.test_inputs, test_labels = scaler.transform(np.array(pd.read_csv('./cover_test.csv'))), np.array(pd.read_csv('./cover_test_label.csv')).reshape(-1,)
        ...
        for digit in self._train_digits:                        # 0 하나밖에 없어서 for문 한번만 돈다
            ...
            self._train_ids_per_label.update({digit: ids})   #  {0:25,36,57 ...} 이런식으로 각 task 숫자의 index ids위치
    ...
    def _gen_dataset(self, test=False):
        ...
        tasks = []     # tasks 별로 나눠서, train은 [(0,), (0,), (0,), (0,)], test는 [(1,), (1,), (1,), (1,)]
        while True:
        ...
        num_inputs_per_meta_batch = (self._batch_size  * self._meta_batch_size)  # meta batch (4)당 input * 2 (10)은 40
        ids, lbls = np.empty((0, num_inputs_per_meta_batch), dtype=np.int32), np.empty((0, num_inputs_per_meta_batch), dtype=np.int32)
        data_size = self._test_size if test else self._train_size         # 트레인 (269)        테스트 (40)
        data_size = min(data_size // num_inputs_per_meta_batch, 1000)     # 트레인 (269 // 40) __ 6 테스트 (40 // 40) __1
        
        for i in range(data_size):    # 총 한번 데이터셋 크기 6개 만큼 돌면서  -- 왜 6개 만큼 도는거지? 하나만해도 훈련 가능하잖아;
            all_ids, all_labels = np.array([], dtype=np.int32), np.array([], dtype=np.int32) 
            for task in tasks:   # tasks는 [(0,), (0,), (0,), (0,)] 6개 (train set) [(1,), (1,), (1,), (1,)] 1개 (test set)
                task_ids, task_labels = np.array([], dtype=np.int32), np.array([], dtype=np.int32)
                for label in task:       # 여기 label은 실제 정답 라벨
                    ...
                    labels.fill(label)    # 원래 i였는데 i가 아니라 label로 넣어야하지 않나? 내가 바꿔줌 -- i로 하면 무조건 0임..
        ...
        return tf.data.Dataset.from_tensor_slices((all_inputs, all_labels))    # (40, 30) (40,)

    def build_inputs_and_labels(self, handle):
        slice_size = (self._batch_size // 2) 
        input_batches, label_batches = self._gen_metadata(handle)    # 실제 정답 label 값 여기서 받아와서 acc 출력 가능~
        ...
        return input_a, label_a, input_b, label_b, real_a, real_b  # (7, 5, 10) (7, 5, 10, 1) (7, 5, 10) (7, 5, 10, 1) (7, 5) (7, 5)