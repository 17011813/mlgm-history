...
class Vae(Model):
    ...
    def build_loss(self, labels, logits):      # 복원된 예측 logits 결과로 라벨과 비교해서 loss 구함    # KL은 (100,)
        ...
        mse_loss = tf.reduce_sum(tf.square(labels-logits), 1)       # mse_loss가 reconstruction error임 -- 얘까지는 (100, 1)인데
        loss = tf.reduce_mean(mse_loss + KL_divergence)             # MLP_VAE 식으로 loss 구하는거 바꿈 (MSE + KLD)  -- 얘부터 () 꼴
        
        #mse_loss = tf.reshape(mse_loss, shape = [100,])    # (100,)로 바꿈   -- 2022.01.27에 K를 5에서 100으로 바꿨기 땜에 여기도 바꿔줌
        #test_loss = mse_loss + KL_divergence               # (100,)랑 (100,) 끼리 더해서 (100,)꼴
        return loss, mse_loss                               # mse_loss는 (100,)로 각각 loss 출력함 / test_loss는 (100,)로 모든 애들 loss 출력~  
