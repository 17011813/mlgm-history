...
class Model:
    ...
    def build_accuracy(self, labels, output, name=None):
        with tf.variable_scope(name, default_name=self._name + "_accuracy", values=[labels, output]):
            _, acc = tf.metrics.accuracy(tf.argmax(labels, axis=1), tf.argmax(output, axis=1))
            return acc
