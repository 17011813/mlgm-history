...
class MetaSampler:
    def __init__(self, batch_size, meta_batch_size, inputs):
        # 같은 분포에서 2번 샘플링 하기 때문에 batch size * 2
    ...
    def _gen_metadata(self, handle):
        num_inputs_per_batch = (self._batch_size)  # 배치마다 들어가는 입력 수 = 5
        self._train_iterator = self._train_dataset.make_initializable_iterator()
        self._test_iterator = self._test_dataset.make_initializable_iterator()
        ...
        for i in range(self._meta_batch_size):
            batch_input_sym = meta_batch_sym[0][i * num_inputs_per_batch:(i + 1) * num_inputs_per_batch]
            batch_label_sym = meta_batch_sym[1][i * num_inputs_per_batch:(i + 1) * num_inputs_per_batch]
            shuffle_batch_input_sym = []
            shuffle_batch_label_sym = []
            for k in range(self._batch_size):
                class_ids = tf.range(0, 1)
            ...
            all_label_batches.append(shuffle_batch_label_sym)        # 섞어도 되나?
