...
class Maml:
    ...
    def _build(self):        
        with self._sess.graph.as_default():   # (3, 100, 8) (3, 100, 8, 1) (3, 100, 8) (3, 100, 8, 1) (3, 100) (3, 100) 잘 불러오구여
            ...
            def task_metalearn(args):
                ...
                for i in range(self._num_updates):       # print(label_b.shape)  # (5,10,1)
                    ...
                    print_test_loss = test_loss           # 맨 마지막꺼만 가져다가 test_loss로 사용
    ...
    def _test(self, test_itr, test_handle, global_step):
        ...
        y_true, y_pred = [], []            
        for j in range(test_itr):      # input imgs는 (3, 100, 8) 원래 꼴
            try:        # (5, 3, 100, 8, 1) gen_imgs는 input_b (query set)으로 부터 얻은 출력값
                input_imgs, gen_imgs, loss_a, losses_b, mse_loss, test_loss, real_a, real_b = self._compute_metatest(test_handle) # 최종으로 받을때는 test_losses로 이름 바꿔줌
                #print(np.array(input_imgs).shape, np.array(gen_imgs).shape, np.array(loss_a).shape, np.array(losses_b).shape)
                #print("**************************************real b : ",real_b)

                loss_a = np.mean(loss_a)                     # (3,) --> ()
                #mse_loss = np.mean(mse_loss)                # (  이렇게 굳이 안해도 밑에 threshold될때 알아서 됨
                #print(np.array(test_loss).shape)            # (  모든 애들 loss 각각 출력
                #print(losses_b.shape)                       # (5, 3)  정확하게 맞아~~~
