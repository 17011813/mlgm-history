...
class MnistMetaSampler(MetaSampler):
    def __init__(
        ...
        self.train_inputs, train_labels = scaler.fit_transform(np.array(pd.read_csv('./data/MTS_train.csv'))), np.array(pd.read_csv('./data/MTS_train_label.csv')).reshape(-1,)
        self.test_inputs, test_labels = scaler.transform(np.array(pd.read_csv('./data/MTS_test.csv'))), np.array(pd.read_csv('./data/MTS_test_label.csv')).reshape(-1,)
    ...
    def _gen_dataset(self, test=False):
        ...
        num_inputs_per_meta_batch = (self._batch_size  * self._meta_batch_size)  # meta batch (3)당 input (5) * 2 (10)은 30
        ids, lbls = np.empty((0, num_inputs_per_meta_batch), dtype=np.int32), np.empty((0, num_inputs_per_meta_batch), dtype=np.int32)
        data_size = self._test_size if test else self._train_size         # 트레인 (10880)              테스트 (1192)
        data_size = min(data_size // num_inputs_per_meta_batch, 1000)     # 트레인 (10880 // 30) __ 362 테스트 (1192 // 30) __ 39
        ...
        return tf.data.Dataset.from_tensor_slices((all_inputs, all_labels))    # train (362, 30, 8) (362, 30) // test (39, 30, 8) (39, 30)
    ...
    def build_inputs_and_labels(self, handle):
        ...
        return input_a, label_a, input_b, label_b, real_a, real_b  # (3, 5, 8) (3, 5, 8, 1) (3, 5, 8) (3, 5, 8, 1) (3, 5) (3, 5)