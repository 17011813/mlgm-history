...
class Model:
    ...
    def build(self):
        ...
        self._y = tf.placeholder(**self._param_out)     # 원래 라벨
        self._out = self.build_forward_pass(self._x)    # 복원된 출력 결과 logits 값이 _out임
        loss_sym = self.build_loss(self._y, self._out)  # 원래 오리지널 x, logit 넣고 
    ...
    def _set_tensors(self, layer_in, layer, use_tensors):
        supported_types = [ tf.keras.layers.Dense, tf.keras.layers.Conv2D ]   # 여기에 VAE에서 쓰는 layer들 선언
    ...
    def build_forward_pass(self, input_tensor, use_tensors=None, name=None):     # Model layers
