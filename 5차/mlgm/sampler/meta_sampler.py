...
class MetaSampler:
    def __init__(self, batch_size, meta_batch_size):   # 같은 분포에서 2번 샘플링하기 때문에 *2 해줌
        ...
        self._ids_per_label = {}        
        self._train_dataset = self._gen_dataset()    # 여기서는 (362, 30, 8) (362, 30) // test (39, 30, 8) (39, 30) 다 받는데
    ...
    def _gen_metadata(self, handle):
        num_inputs_per_batch = self._batch_size      # 배치마다 들어가는 입력 수 = batch_size_K (5) * 2 = 10
        ...
        for i in range(self._meta_batch_size):
            batch_input_sym = meta_batch_sym[0][i * num_inputs_per_batch:(i + 1) * num_inputs_per_batch]   # (10, 30)
            batch_label_sym = meta_batch_sym[1][i * num_inputs_per_batch:(i + 1) * num_inputs_per_batch]   # (10, )
        ...
        #print(tf.stack(all_input_batches).shape, tf.stack(all_label_batches).shape)
        return tf.stack(all_input_batches), tf.stack(all_label_batches)   # (3, 10, 8) (3, 10) 여기 라벨은 실제 0 또는 1의 라벨           
