...
def main():
    ...
    with tf.Session() as sess:
        ...
        maml = Maml(
            ...
            sess,
            logger,
            num_updates=5,            # inner gradient 업데이트 횟수 -- 과적합땜에 줄여 5 이상은 의미 없음
            update_lr=0.0005,
            meta_lr=0.0001,
            outliers_fraction=0.07)   # threshold 설정 비율
        
        maml.train( train_itr=60000, test_itr=1, test_interval=100 )  # iter는 outer 총 예시 task 작업 수 -- 논문에서는 최소 1만 이상,,,?!
        """              
        maml.test( test_itr=3, restore_model_path='./data/maml_vae/maml_vae_01_27_11_12/itr_53000_loss0.0179')               
        """
